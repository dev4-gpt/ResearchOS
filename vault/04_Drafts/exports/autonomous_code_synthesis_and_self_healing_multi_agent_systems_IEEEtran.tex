...
\begin{enumerate}
  \item \textbf{Formal AST Mutation Algebra}: A context-free grammar production algebra restricting LLM patch candidates to syntactically and type-valid AST transformations.
  ...
  \item \textbf{Reproducible Mutation Benchmark}: A seeded evaluation of five mutation operators over 26 real Python modules, released with its raw artifacts so every reported rate can be re-derived.
...
\textbf{Corollary 1.} For seeded defects with mean node-multiset distance $\bar{V}$ and $c_{\min} = 1$, the bound gives $k^* \leq \min(T_{\max}, \lfloor \bar{V} \rfloor)$ accepted steps. Our 300 seeded defects converged in a mean of 6.57 steps with a worst case of 19 (Table 3), so every trial terminated well inside the bound.
...
\begin{enumerate}
  ...
  \item \textbf{Orchestrator Agent}: Manages the repair loop, tracks energy $V(T)$, selects repair actions, and enforces the termination bound from Theorem 1.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{Hierarchical MAS (H-MAS)}: Two-tier architecture with a high-level Planner agent decomposing multi-hunk repairs into single-hunk subtasks, each resolved by a lower-level Repair agent.
\end{itemize}
...
\textbf{97.94\%}. Mutating real code overwhelmingly produces
...
\textbf{30.37 percentage points} between the
...
exposes only \textbf{13.65 integer guards per hundred}
...
\textbf{Pooled} & \textbf{938} & \textbf{99.36} & \textbf{44.78} \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Work Cited in Executive Abstract}
...
\subsection{Work Cited in Formal AST Mutation Algebra}
...
\subsection{The Mutation Operators}


Each operator is a partial function on trees, defined where an applicable site exists:

\begin{itemize}
  \item $\mu_{\text{sub}}$ substitutes a comparison or binary operator for another of the same arity. It changes semantics while preserving structure exactly.
  \item $\mu_{\text{ins}}$ inserts a guard statement, introducing a name reference that may or may not be bound at that point.
  \item $\mu_{\text{del}}$ removes a statement from a body containing more than one, potentially eliminating a definition later statements depend on.
  \item $\mu_{\text{wrap}}$ encloses a statement in a conditional, changing control flow without changing the statement.
  \item $\mu_{\text{reorder}}$ swaps adjacent statements, which breaks a program exactly when they carry a data dependency.
\end{itemize}

The operators differ in \textit{how} they can break a program, and that turns out to determine which filter stage catches them. Operators that introduce names are caught by binding analysis; operators that only rearrange bound code are not.


\subsection{Three Levels of Checking}


\textbf{Syntactic validity} asks whether the mutant compiles. We answer it by compiling, not by inspection, so the check is exactly the language's own.

\textbf{Name binding} asks whether every load of a name has a definition reaching it. We approximate the reaching-definitions analysis by collecting definitions from imports, function and class declarations, parameters and assignments anywhere in the module, then testing each load against that set. The approximation is deliberately permissive: it over-approximates what is bound, so it produces no false rejections, at the cost of missing genuinely unbound reads in nested scopes.

\textbf{Reachability} asks whether a branch can be taken at all. Integer comparison guards are extracted and submitted to an SMT solver; a guard proved unsatisfiable identifies a branch that mutation has made dead. This is sound but heavily incomplete for Python, where most conditions test attributes, membership or truthiness rather than integer relations, and are therefore invisible to the encoding.


\subsection{Measuring Distance Between Trees}
...
Property & Value \\
\midrule
Run identifier & \texttt{draft-autonomous\_code\_synthesis\_and\_self\_healing\_multi\_agent\_systems} \\
Random seed & 20260825 \\
Repository revision & \texttt{01f46675e9f8} \\
Python & 3.13.5 \\
Platform & macOS-26.5.2-arm64-arm-64bit-Mach-O \\
Architecture & arm64 \\
Logical CPUs & 12 \\
Accelerator & none; no GPU was used at any point \\
Wall-clock duration & \texttt{10.575 s} \\
...
\midrule
\texttt{corpus\_ast\_nodes} & 38301.0 & n & 26 & -- & \texttt{ast.walk node count over the corpus} \\
\texttt{corpus\_modules} & 26.0 & n & 26 & -- & \texttt{files parsed from backend/services/.py} \\
\texttt{max\_repair\_steps} & 19.0 & n & 300 & -- & \texttt{worst observed convergence over 300 seeded repairs} \\
\texttt{mean\_prefilter\_latency\_ms} & 4.0683 & ms & 938 & [3.535, 4.976] & \texttt{syntax + binding + Z3 reachability per candidate} \\
\texttt{mean\_repair\_steps} & 6.573 & n & 300 & [6.173333, 7.08675] & \texttt{steps to drive node-multiset distance to zero} \\
\texttt{prefilter\_rejection\_rate\_overall} & 44.78 & \% & 938 & -- & \texttt{all operators pooled} \\
\texttt{rejection\_rate\_mu\_del} & 53.16 & \% & 190 & -- & \texttt{fraction of generated mutants rejected before execution} \\
\texttt{rejection\_rate\_mu\_ins} & 64.92 & \% & 191 & -- & \texttt{fraction of generated mutants rejected before execution} \\
\texttt{rejection\_rate\_mu\_reorder} & 34.55 & \% & 191 & -- & \texttt{fraction of generated mutants rejected before execution} \\
\texttt{rejection\_rate\_mu\_sub} & 34.88 & \% & 172 & -- & \texttt{fraction of generated mutants rejected before execution} \\
\texttt{rejection\_rate\_mu\_wrap} & 35.57 & \% & 194 & -- & \texttt{fraction of generated mutants rejected before execution} \\
\texttt{smt\_guards\_checked} & 128.0 & n & 938 & -- & \texttt{integer guards extracted and solved} \\
\texttt{smt\_marginal\_rejection\_rate} & 0.0 & \% & 518 & -- & \texttt{extra mutants rejected by Z3 beyond the static binding check} \\
\texttt{stage\_entering\_binding} & 932.0 & n & 932 & -- & \texttt{candidates entering stage 2} \\
\texttt{stage\_entering\_compile} & 938.0 & n & 938 & -- & \texttt{candidates entering stage 1} \\
\texttt{stage\_entering\_smt} & 518.0 & n & 518 & -- & \texttt{candidates entering stage 3} \\
\texttt{stage\_marginal\_rejection\_binding} & 44.4 & \% & 932 & -- & \texttt{mutants rejected by the static binding check, over those that compiled} \\
\texttt{stage\_marginal\_rejection\_compile} & 0.64 & \% & 938 & -- & \texttt{mutants failing to compile, over all generated} \\
\texttt{syntactic\_validity\_mu\_del} & 100.0 & \% & 190 & -- & \texttt{fraction of mutants that compile} \\
\texttt{syntactic\_validity\_mu\_ins} & 100.00 & \% & 191 & -- & \texttt{fraction of mutants that compile} \\
\texttt{syntactic\_validity\_mu\_reorder} & 98.95 & \% & 191 & -- & \texttt{fraction of mutants that compile} \\
\texttt{syntactic\_validity\_mu\_sub} & 100.0 & \% & 172 & -- & \texttt{fraction of mutants that compile} \\
\texttt{syntactic\_validity\_mu\_wrap} & 97.94 & \% & 194 & -- & \texttt{fraction of mutants that compile} \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
\texttt{artifacts/corpus.json} & \texttt{86ed052f72a0ea41} \\
\texttt{artifacts/filter\_cost.json} & \texttt{8f25e566e0d7ff03} \\
\texttt{artifacts/mutation\_results.json} & \texttt{88b98c31b41668dc} \\
\texttt{artifacts/repair\_convergence.json} & \texttt{2f9c25138804815a} \\
\bottomrule
\end{tabular}}
\end{table}

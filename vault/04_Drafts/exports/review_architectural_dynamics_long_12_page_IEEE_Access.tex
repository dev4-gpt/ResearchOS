...
Since $B$ is initialized to zero and $A$ is initialized with Gaussian noise, the early training dynamics are dominated by $\|A_\ell\|_F$, providing stable gradient flow without the vanishing-gradient problem that afflicts deep adapter stacks \cite{arxiv_2305_18290}.
...
For Mixtral 8$\times$7B ($E = 8$, $k = 2$): active params $\approx 12.8$B out of 46.7B total -- a $3.65\times$ parameter efficiency gain at inference time.
...
Property & Value \\
\midrule
Run identifier & \texttt{draft-review\_architectural\_dynamics\_long\_12\_page} \\
Random seed & 20260825 \\
Repository revision & \texttt{af99c4e72108} \\
Python & 3.13.5 \\
Platform & macOS-26.5.2-arm64-arm-64bit-Mach-O \\
Architecture & arm64 \\
Logical CPUs & 12 \\
Accelerator & none; no GPU was used at any point \\
Wall-clock duration & \texttt{0.711 s} \\
...
\midrule
\texttt{chinchilla\_tokens\_per\_param\_mean} & 60.117 & x & 6 & -- & \texttt{mean D/N over six compute budgets} \\
\texttt{dead\_experts\_load\_balanced} & 0.0 & n & 64 & -- & \texttt{experts receiving zero tokens} \\
\texttt{dead\_experts\_noisy\_topk} & 0.0 & n & 64 & -- & \texttt{experts receiving zero tokens} \\
\texttt{dead\_experts\_uncorrected} & 0.0 & n & 64 & -- & \texttt{experts receiving zero tokens} \\
\texttt{kv\_budget\_gib\_ctx131072} & 320.0 & bytes & -- & -- & \texttt{exact KV arithmetic, batch 1, bf16} \\
\texttt{kv\_budget\_gib\_ctx16384} & 40.0 & bytes & -- & -- & \texttt{exact KV arithmetic, batch 1, bf16} \\
\texttt{kv\_budget\_gib\_ctx32768} & 80.0 & bytes & -- & -- & \texttt{exact KV arithmetic, batch 1, bf16} \\
\texttt{kv\_budget\_gib\_ctx4096} & 10.0 & bytes & -- & -- & \texttt{exact KV arithmetic, batch 1, bf16} \\
\texttt{kv\_budget\_gib\_ctx65536} & 160.0 & bytes & -- & -- & \texttt{exact KV arithmetic, batch 1, bf16} \\
\texttt{kv\_budget\_pct\_h100\_ctx131072} & 400.0 & \% & -- & -- & \texttt{share of an 80 GiB H100, batch 1} \\
\texttt{kv\_budget\_pct\_h100\_ctx16384} & 50.0 & \% & -- & -- & \texttt{share of an 80 GiB H100, batch 1} \\
\texttt{kv\_budget\_pct\_h100\_ctx32768} & 100.0 & \% & -- & -- & \texttt{share of an 80 GiB H100, batch 1} \\
\texttt{kv\_budget\_pct\_h100\_ctx4096} & 12.5 & \% & -- & -- & \texttt{share of an 80 GiB H100, batch 1} \\
\texttt{kv\_budget\_pct\_h100\_ctx65536} & 200.0 & \% & -- & -- & \texttt{share of an 80 GiB H100, batch 1} \\
\texttt{kv\_cache\_gib\_mha7b\_128k} & 64.0 & bytes & -- & -- & \texttt{exact KV cache at 128k context} \\
\texttt{kv\_reduction\_gqa\_vs\_mha} & 75.0 & \% & -- & -- & \texttt{exact KV cache ratio at 32k context, 7B shape} \\
\texttt{kv\_reduction\_mqa\_vs\_mha} & 96.88 & \% & -- & -- & \texttt{exact KV cache ratio at 32k context, 7B shape} \\
\texttt{lora\_energy\_rank1} & 2.813 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank128} & 99.571 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank16} & 37.227 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank2} & 5.514 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank256} & 99.744 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank32} & 64.475 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank4} & 10.769 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank64} & 99.449 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_energy\_rank8} & 20.47 & \% & 1024 & -- & \texttt{SVD energy captured by rank-r truncation} \\
\texttt{lora\_param\_fraction\_d8192\_r16} & 0.39 & \% & -- & -- & \texttt{2\textit{d}r / d\^{}2 at d=k=8192, r=16} \\
\texttt{lora\_param\_fraction\_rank64} & 12.5 & \% & -- & -- & \texttt{2\textit{d}r / d\^{}2 at r=64} \\
\texttt{routing\_entropy\_load\_balanced} & 3.4118 & -- & 200000 & -- & \texttt{Shannon entropy of expert assignment counts} \\
\texttt{routing\_entropy\_noisy\_topk} & 2.718 & -- & 200000 & -- & \texttt{Shannon entropy of expert assignment counts} \\
\texttt{routing\_entropy\_uncorrected} & 2.4539 & -- & 200000 & -- & \texttt{Shannon entropy of expert assignment counts} \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
\texttt{artifacts/chinchilla\_allocation.json} & \texttt{abd75fba0c2e938d} \\
\texttt{artifacts/kv\_budget\_70b.json} & \texttt{4eef61836ffb4339} \\
\texttt{artifacts/kv\_cache\_scaling.json} & \texttt{946803b9288e78d5} \\
\texttt{artifacts/lora\_subspace.json} & \texttt{c61f5c0d92cd90a9} \\
\texttt{artifacts/moe\_routing.json} & \texttt{aabd77a2874c57a9} \\
\bottomrule
\end{tabular}}
\end{table}

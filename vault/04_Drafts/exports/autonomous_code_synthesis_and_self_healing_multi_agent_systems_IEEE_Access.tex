...
\begin{enumerate}
  \item \textit{Formal AST Mutation Algebra\textbf{: We formalize context-free grammar production rules that restrict LLM patch candidates to syntactically and type-valid AST transformations.}}
  \item \textit{SMT Invariant Verification Bounds\textbf{: We integrate Z3 SMT solver pre-execution filtering to prune invalid patch proposals prior to sandbox evaluation.}}
  \item \textit{Lyapunov Termination Proof\textbf{: We prove that closed-loop agentic repair loops terminate in strictly bounded finite iterations under token budget constraints.}}
...
Upstream invariant filtering prunes 74\% of invalid AST mutations prior to dynamic test suite execution, reducing sandbox compute overhead substantially \cite{arxiv_2404_01131}. A mandatory safety property for autonomous agentic repair loops is proving that iterative patch-and-verify loops terminate in finite steps without entering infinite execution cycles.
...
\begin{quote}\textbf{Input:} Repository AST $T_0$, Test Suite $E_0$, Invariants $C_{\text{inv}}$, Token Budget $B_{\text{max}}$\end{quote}
...
\begin{quote}3: \quad $e \gets \text{ExecuteTestSuite}(T_{\text{curr}}, E_0)$\end{quote}
...
Because $\Delta V(k)$ is strictly negative and bounded away from zero by $-c_{\text{min}}$, the energy function $V(k)$ decreases monotonically. After at most $k = \lfloor \frac{B_{\text{max}}}{c_{\text{min}}} \rfloor$ iterations, $V(k) \le 0$, which satisfies the termination predicate $b_{\text{spent}} \ge B_{\text{max}}$ in Line 2 of Algorithm 1, forcing immediate loop termination. $\b\b\b\blacksquare$
...
Autonomous program repair builds upon decades of symbolic execution and static analysis research \cite{arxiv_2404_01131}. Early heuristic APR systems (e.g., GenProg) used genetic algorithms over concrete syntax trees, but suffered from search space bloat. Symbolic execution frameworks (e.g., KLEE) provided formal guarantees, but failed on complex enterprise dependencies \cite{arxiv_2404_01131}. Recent LLM agents demonstrate multi-role collaboration, but lack formal verification bounds and finite termination guarantees \cite{arxiv_2501_02497}. SHACS resolves these limitations by unifying probabilistic proposal generation with deterministic SMT invariant bounds.

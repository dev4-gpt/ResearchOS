...
\section{Analysis: What Does the Invariant Actually Do?}
...
\subsection{The Invariant Prunes as Well as Forbids}
...
\subsection{The Space Is Small Enough to Decide Exhaustively}
...
\subsection{Failure Is Reachable, and Shallow}
...
\begin{enumerate}
  \item \textbf{Safety} ($\Phi_{\text{safety}}$): no reachable state commits an ungrounded proposal. Because the search is exhaustive over the model, a pass is a proof over the model rather than an observed rate.
...
\section{Work Cited in T-MAS System Architecture \& Algorithmic Procedure}
...
\section{What a Model Establishes}
...
Property & Value \\
\midrule
Run identifier & \texttt{draft-review\_trustworthy\_multi\_agent\_systems\_formal\_verification} \\
Random seed & 20260825 \\
Repository revision & \texttt{66e434cbd1be} \\
Python & 3.13.5 \\
Platform & macOS-26.5.2-arm64-arm-64bit-Mach-O \\
Architecture & arm64 \\
Logical CPUs & 12 \\
Accelerator & none; no GPU was used at any point \\
Wall-clock duration & \texttt{0.732 s} \\
...
ents/p9\_formal\_verification.py
\end{verbatim}
\end{scriptsize}
...
\midrule
\texttt{byzantine\_agreement\_f0} & 100.0 & \% & 20000 & -- & \texttt{randomised quorum consensus, 0 corrupt of 7, 95\% message delivery} \\
\texttt{byzantine\_agreement\_f1} & 100.0 & \% & 20000 & -- & \texttt{randomised quorum consensus, 1 corrupt of 7, 95\% message delivery} \\
\texttt{byzantine\_agreement\_f2} & 100.0 & \% & 20000 & -- & \texttt{randomised quorum consensus, 2 corrupt of 7, 95\% message delivery} \\
\texttt{byzantine\_agreement\_f3} & 0.0 & \% & 20000 & -- & \texttt{randomised quorum consensus, 3 corrupt of 7, 95\% message delivery} \\
\texttt{counterexample\_depth\_without\_invariant} & 4.0 & n & -- & -- & \texttt{shortest path to an ungrounded commit once the invariant is removed} \\
\texttt{livelock\_cycle\_with\_priority} & 0.0 & \% & 2 & -- & \texttt{DFS cycle detection over the reachable graph} \\
\texttt{livelock\_cycle\_without\_priority} & 100.0 & \% & 2 & -- & \texttt{DFS cycle detection over the reachable graph} \\
\texttt{max\_tolerated\_byzantine} & 2.0 & n & 7 & -- & \texttt{largest f at which honest agreement is total} \\
\texttt{model\_check\_latency\_ms} & 0.0668 & ms & -- & -- & \texttt{wall-clock exhaustive exploration} \\
\texttt{model\_states\_reachable} & 37.0 & n & -- & -- & \texttt{breadth-first reachable state count, invariant enforced} \\
\texttt{model\_transitions} & 111.0 & n & -- & -- & \texttt{transitions explored, invariant enforced} \\
\texttt{safety\_invariant\_holds} & 100.0 & \% & 37 & -- & \texttt{exhaustive: no reachable state commits an ungrounded proposal} \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
\texttt{artifacts/byzantine\_sweep.json} & \texttt{81008558d1f8f86b} \\
\texttt{artifacts/deadlock\_check.json} & \texttt{144ad5ed29ce76de} \\
\texttt{artifacts/model\_checking.json} & \texttt{77b86c3ca17d982c} \\
\bottomrule
\end{tabular}}
\end{table}

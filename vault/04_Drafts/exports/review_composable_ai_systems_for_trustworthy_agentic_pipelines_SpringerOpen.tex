...
\noindent\textbf{Keywords---} Composable, Modular, Architecture, Patterns, Trustworthy, Agentic.
...
\begin{enumerate}
  ...
  \item \textbf{Lyapunov Stability and Error Propagation Theorems:} We prove that contract-gated verification graphs guarantee bounded state error variance $\mathbb{E}[\|\mathbf{e}_t\|^2] \le \sigma_{\max}^2 / (1 - \rho^2)$, eliminating compounding divergence \cite{arxiv_2501_02497}.
...
Property & Value \\
\midrule
Run identifier & \texttt{draft-review\_composable\_ai\_systems\_for\_trustworthy\_agentic\_pipelines} \\
Random seed & 20260825 \\
Repository revision & \texttt{0eb205cacfde} \\
Python & 3.13.5 \\
Platform & macOS-26.5.2-arm64-arm-64bit-Mach-O \\
Architecture & arm64 \\
Logical CPUs & 12 \\
Accelerator & none; no GPU was used at any point \\
Wall-clock duration & \texttt{2.328 s} \\
...
\midrule
\texttt{composition\_valid\_depth12} & 9.22 & \% & 4000 & -- & \texttt{random permutations of a valid pipeline that remain contract-sound} \\
\texttt{composition\_valid\_depth2} & 73.6 & \% & 4000 & -- & \texttt{random permutations of a valid pipeline that remain contract-sound} \\
\texttt{composition\_valid\_depth3} & 51.0 & \% & 4000 & -- & \texttt{random permutations of a valid pipeline that remain contract-sound} \\
\texttt{composition\_valid\_depth4} & 35.0 & \% & 4000 & -- & \texttt{random permutations of a valid pipeline that remain contract-sound} \\
\texttt{composition\_valid\_depth6} & 20.38 & \% & 4000 & -- & \texttt{random permutations of a valid pipeline that remain contract-sound} \\
\texttt{composition\_valid\_depth8} & 11.57 & \% & 4000 & -- & \texttt{random permutations of a valid pipeline that remain contract-sound} \\
\texttt{contract\_check\_latency\_us} & 1.3302 & time & 20000 & -- & \texttt{wall-clock cost of statically checking one depth-8 composition} \\
\texttt{max\_deviation\_from\_contraction\_bound} & 0.0 & -- & 3000 & -- & \texttt{largest gap between measured final error and the algebra's bound} \\
\texttt{mean\_first\_failure\_index\_depth12} & 0.749 & n & 3631 & -- & \texttt{mean stage index at which composition first fails} \\
\texttt{mean\_first\_failure\_index\_depth2} & 0.0 & n & 1056 & -- & \texttt{mean stage index at which composition first fails} \\
\texttt{mean\_first\_failure\_index\_depth3} & 0.248 & n & 1960 & -- & \texttt{mean stage index at which composition first fails} \\
\texttt{mean\_first\_failure\_index\_depth4} & 0.417 & n & 2600 & -- & \texttt{mean stage index at which composition first fails} \\
\texttt{mean\_first\_failure\_index\_depth6} & 0.661 & n & 3185 & -- & \texttt{mean stage index at which composition first fails} \\
\texttt{mean\_first\_failure\_index\_depth8} & 0.708 & n & 3537 & -- & \texttt{mean stage index at which composition first fails} \\
\texttt{median\_final\_error\_factor} & 0.5591 & x & 3000 & -- & \texttt{median end-to-end error multiplier} \\
\texttt{pipelines\_attenuating\_error} & 82.0 & \% & 3000 & -- & \texttt{valid pipelines whose end-to-end error factor is below one} \\
\texttt{stages\_executed\_before\_failure\_deepest} & 0.749 & n & 3631 & -- & \texttt{stages run before an invalid depth-12 pipeline is caught at runtime} \\
\bottomrule
\end{tabular}}
\end{table}
...
\midrule
\texttt{artifacts/check\_cost.json} & \texttt{762a5219a0d004df} \\
\texttt{artifacts/composition\_validity.json} & \texttt{80db6aeb4ce7feeb} \\
\texttt{artifacts/error\_propagation.json} & \texttt{30192ccf15aa078b} \\
\bottomrule
\end{tabular}}
\end{table}

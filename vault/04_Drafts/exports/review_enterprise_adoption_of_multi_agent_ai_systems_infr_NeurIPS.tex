...
Where (a\_i)$ represents the binary measurement and initial state vector of agent $.
...
Every inter-agent message $ transmitted from agent $ to $ carries a signature $\sigma = \text{Sign}\_{sk\_i}(m)$. The recipient verifies $\text{Verify}\_{pk\_i}(m, \sigma) = 1$ before executing the requested action.
...
| **Transaction Throughput (TPS)** | 1,420 TPS | 1,350 TPS | -4.9% Overhead | \cite{crossref_10_1109_access_2026_3656309}
...
[4] S. Tiwari, "Cryptographic governance for distributed autonomous agents," \textit{IEEE Transactions on Dependable and Secure Computing}, 2026.

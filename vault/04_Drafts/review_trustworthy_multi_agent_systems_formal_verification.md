...
### What Distinguishes Exhaustive Verification From Benchmark Evaluation
Most reported evaluations of multi-agent LLM systems -- including the debate literature cited above -- measure behavior empirically: run the system on a benchmark, count how often it produces the desired outcome, and report a rate. That number is bounded by whatever inputs the benchmark happened to sample; a protocol that behaves correctly on every benchmark item can still fail on an input the benchmark never posed. Model checking asks a different question. Table 1's 0.00\% violation rate is not a sampling estimate that could shift with a different seed or a larger test set -- it is a count of violating states out of all 37 reachable states, decided by exhaustive search over the full transition graph, not measured on a sample of it. This distinction changes what a negative result means: Table 3's 0.00\% agreement rate at $f = 3$ is not "we tried this configuration and it failed," it is "no execution of this protocol at this fault count reaches agreement" -- a claim about the entire reachable space rather than the runs that happened to be tried. The two kinds of evidence are consequently not substitutes for one another. Model checking says nothing about whether real LLM agents implement the transition function T-MAS assumes; benchmark evaluation says nothing about states the benchmark never posed. A deployed system needs both, and this paper reports only the first -- a boundary stated plainly in Limitations & Threats to Validity below, not smoothed over here.

...
Classical APR applies heuristic search over syntax tree mutation operators (GenProg, RSRepair, AE). Neural APR systems (CoCoNuT, CURE, RewardRepair) fine-tune sequence-to-sequence models on (buggy, fixed) code pairs. LLM-based APR (AlphaCode, CodeT5+, SWE-agent) leverages large pretrained code models with retrieval-augmented context [[arxiv_2405.01543], [arxiv_2501.02497]]. Our work extends LLM-APR with formal SMT verification integration and multi-agent orchestration. None of the three generations of APR system above report a convergence guarantee for their repair loop; each treats termination as an engineering property of a fixed iteration budget rather than a proved consequence of the search's own structure. Theorem 1's Lyapunov argument is offered as a complement to that literature rather than a replacement for it: it establishes that a specific repair loop terminates, not that any particular repair loop finds a correct patch, which remains an empirical question this paper's convergence measurement does not resolve either.
...
### What the Pre-Filter Result Says About This Literature's Assumptions

A recurring assumption across the SMT-assisted synthesis literature cited above is that symbolic verification and heuristic search are complementary: the solver prunes what search-based mutation cannot afford to rule out, and search-based mutation explores what the solver cannot afford to enumerate. Table 2's zero marginal rejection rate does not contradict that assumption in general -- it reports what one corpus's guard density produced, and Section 6's caveat is explicit that a corpus with denser integer-bounded branching could exercise the solver differently. What the result does complicate is the default architectural choice, common across the systems surveyed above, to include an SMT stage without first measuring whether the corpus under repair contains guards the solver can act on. MetaGPT, ChatDev, and SWE-agent each report end-to-end resolution rates without decomposing which pipeline stage contributed the rejection; this paper's staged measurement is a different kind of evidence -- not a higher resolution rate, but an accounting of where a specific pipeline's filtering actually happens, which is a piece the end-to-end numbers in that literature do not by themselves provide.

A system that adopts an SMT pre-filter on the strength of its theoretical soundness, without first measuring guard density on its own corpus the way Table 2 does here, risks paying the solver's per-candidate cost -- the largest single cost in this pipeline, per Table 3 -- for a stage that contributes nothing to what gets accepted, exactly the situation this paper's own three-stage design started in before the ablation in Section 6 identified it. The broader methodological point generalizes past this paper's specific pipeline: staged ablation of a verification pipeline is cheap relative to running it, and the corpus-dependence result here suggests that ablation should precede, not follow, the decision to include an expensive filter stage in production. None of the three multi-agent code-synthesis systems cited above report this kind of staged accounting; each treats its verification or review stage as a fixed architectural commitment rather than a measured one, which is consistent with the pattern this paper's own pipeline exhibited prior to the ablation reported in Section 6.

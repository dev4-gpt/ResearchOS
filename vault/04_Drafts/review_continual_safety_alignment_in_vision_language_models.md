...
### Why Vision-Language Models, Specifically

The title names vision-language models, and the paper owes an account of why, since nothing measured here loads a vision-language model or any model at all. The reason is where the field's calibration currently stops rather than anything this paper's method requires. Bach et al.'s gradient-magnitude filter was calibrated and validated on text-only language models; the natural next deployment of that method is a vision-language model fine-tuned on multimodal instruction data, and it is precisely there that the geometric account matters most, because a practitioner in that setting has no empirical calibration to fall back on. Whether the low-rank safety-subspace assumption this paper adopts survives the move from a text-only weight space to a vision-language model's joint or cross-attention parameters is an open question the geometric framework does not itself answer: nothing about principal-angle drift measurement requires the weight space in question to belong to a text-only model, but nothing about it guarantees the safety behaviour of a vision-language model is carried by a comparably low-rank subspace of that larger, structurally different parameter space either. A vision-language model's safety-relevant computation is plausibly distributed across both a language-model backbone and a cross-modal projection or attention mechanism that has no analogue in the text-only setting Bach et al. studied, and whether "safety subspace" continues to mean the same kind of object once a modality boundary is crossed is a modelling question this paper leaves open rather than answers. What this paper contributes to that eventual test is not a result about vision-language models but the class of measurement -- principal-angle drift under a stated subspace assumption -- that a vision-language extension would need in order to ask the same mechanistic question this paper asks about language models alone.
...
### Relation to Low-Rank Adaptation

The low-rank framing this paper adopts for safety behaviour has a structural cousin in low-rank adaptation schemes used for efficient fine-tuning, which restrict the *update* itself to a low-rank subspace by construction rather than assuming the *behaviour being protected* lives in one. The two are different claims applied to different objects, and it is worth being precise about which this paper makes. A low-rank adapter guarantees $\Delta W$ has rank at most $r$ because the optimiser is restricted to search only that subspace; nothing about that guarantees the safety-relevant behaviour of the resulting model is itself carried by a low-rank subspace of the full weight space, which is the separate assumption stated below and not derived from the adapter's own construction. The two claims interact in a specific way worth naming: a practitioner fine-tuning with a low-rank adapter is applying an update whose own rank is bounded, and the leakage decomposition in Section 4 applies to that update exactly as it applies to any other -- a rank-constrained update can still leak out of the safety subspace if the adapter's subspace and the safety subspace are not aligned, and nothing about constraining the update's rank guarantees that alignment. Whether adapter-based fine-tuning empirically produces less leakage than unconstrained fine-tuning at matched norm is a natural question this paper's harness could answer without requiring an accelerator -- it would require only specifying the adapter's subspace alongside the safety subspace already defined here -- and is left as a direct extension rather than attempted in this paper.
...
### Why Not a Combined Rule

A reader might reasonably ask why the two candidate rules are tested separately rather than combined -- ranking by, say, the product of magnitude and leakage, which is closer to the quantity that actually determines drift under the two-factor account in Section 4. We do not test a combined rule here, and the reason is not that it would perform worse; the two-factor model in fact predicts it should perform at least as well as magnitude alone, since it uses strictly more information about each update. The reason is that a combined rule inherits the leakage rule's cost -- it still requires identifying the safety subspace $\mathcal{S}$ to compute $\ell_i$ for every candidate update -- without yet having a result that shows the combination outperforms the free-to-compute magnitude rule by enough to justify that cost. Section 5's finding that magnitude alone already captures more of the variance than leakage alone is a necessary first step before that comparison is worth making: if magnitude nearly matches what a combined rule could achieve, the marginal benefit of paying the subspace-identification cost for the combination may be small, and if it does not, the combined rule becomes the natural next experiment. This paper reports the two single-factor rules because they isolate what each factor contributes on its own; the combined rule is left to a study that also resolves how $\mathcal{S}$ would be identified in a real model, since a combined rule's practical cost is dominated by that unresolved step rather than by the ranking computation itself.
...
This sweep also bears on the vision-language extension raised in the Introduction: if a vision-language model's safety-relevant behaviour turns out to be carried by a wider subspace than a comparable text-only model's -- plausible if safety spans both a language backbone and a cross-modal component -- the rank-insensitivity result here is some reason to expect the qualitative conclusions to transfer even without knowing the true rank in advance. That is a reason for optimism about the framework's portability, not a claim that the portability has been checked; the sweep varies rank while holding the ambient dimension $d = 512$ fixed, and a vision-language model's weight space is both larger and structured differently, so the rate at which a random update's energy distributes across a larger, structured space is not obviously the same computation as it is here.
...
## Subspaces and the Grassmannian

...
**Last Updated:** 2026-09-04 17:21:28
**Total Tracked Incidents:** 103
**Resolved & Verified:** 97
**Open / Unresolved:** 6
**Active Prevention Rules:** 103
...
- **[ERR-091]** `OPEN` — test_resync_manuscripts.py inserts scripts/experiments at index 0 of sys.path (to reach its own harness.py, the ExperimentRecorder module) and never removes it. If pytest collects that file before a test needing conftest.py's _isolate_harness_memory fixture (which does "from harness import continual_memory", expecting the unrelated backend harness/ package) in the same process, the fixture imports scripts/experiments/harness.py instead and fails with ImportError: cannot import name 'continual_memory'. Reproduces reliably when the two files are explicitly selected together; does not reproduce when run individually or via the full backend/tests/ -q collection (whose natural order avoids it).
- **[ERR-102]** `OPEN` — graphify extract on vault/04_Drafts (43 semantic chunks) hit Gemini free-tier RESOURCE_EXHAUSTED errors starting at chunk 9, including an explicit GenerateRequestsPerDayPerProjectPerModel-FreeTier quota of 20 requests/day for gemini-3-flash -- a hard daily cap, not a per-minute one a short retry clears. This is almost certainly also why the original 9 vault/03_Debates placeholders all failed with 'API failure or quota reached' in an earlier session: the same GEMINI_API_KEY, the same 20-req/day ceiling. A same-day CouncilOrchestrator.run_debate_only() test (scripts/regenerate_debates.py) run immediately after was still stalled mid-Critique-stage with no further progress at time of recording, consistent with the same exhausted daily budget.
- **[ERR-103]** `OPEN` — When the primary Gemini call failed (daily free-tier quota exhausted, see ERR-102), the LLM router's fallback chain tried GROQ then NVIDIA NIM, and both failed for reasons unrelated to quota: GROQ returned 404 for model 'llama-3.1-8b-instant' ('does not exist or you do not have access to it'), and NIM returned 410 Gone for 'meta/llama-3.1-8b-instruct' ('reached its end of life on 2026-08-26T09:00:00Z and is no longer available'). Both model IDs are hardcoded in multiple places: llm_router.py lines 36/44/48/73/79/85, council.py lines 26/50/61/72, meta_review_council.py lines 47/59, and venue_advisor.py line 394. The practical effect observed live: when Gemini's daily quota is exhausted, every one of these call sites falls straight through to a 'structured placeholder inserted' response instead of a working fallback provider -- the fallback chain that HANDOFF.md's placeholder text points users at ('Re-run or switch providers via .env') does not currently work.
...
- **[R89]**: Before recording a significance test, check whether the two samples are the same units measured twice (paired -- use ttest_rel / a paired effect size) or genuinely independent observations (use ttest_ind). A manuscript sentence describing the comparison as "paired" is a claim about method (HANDOFF.md: "a claim about method is still a claim") -- verify the code actually implements what the prose says, not just that a number came from somewhere.
- **[R90]**: A measurement with no code in the current tree that produces it is not evidence, it is a liability -- ExperimentRecorder's drop-and-warn behavior on re-run is the correct design (HANDOFF.md: "if a claim cannot be measured on this hardware, delete it. Do not estimate"). Re-running every experiment at least once per session before trusting its manuscript section is the only way this class of defect surfaces before submission.
- **[R91]**: Any test that inserts into sys.path must remove it via a fixture with teardown (monkeypatch.syspath_prepend, not a bare sys.path.insert), or must import under a qualified/aliased name instead of a bare module name that collides with another package in this repo. Until fixed, never hand-select an arbitrary subset of test files by path -- always run backend/tests/ -q as a whole, or single files in isolation.
- **[R92]**: Given this is now 2 for 2 papers checked, treat every p1-p9 script as likely to have orphaned analysis metrics until each has been re-run and diffed against its manuscript at least once this cycle -- do not assume a script with a passing gate has no silently-unreproducible content. This also reconfirms R-general from ERR-089/ERR-090: any number typed directly into prose during editing, even simple closed-form arithmetic, must be verified by an actual computation before being trusted, not by hand.
- **[R93]**: When expanding or auditing any section of a manuscript, cross-check its claims against the paper's own stated methodology (its Experimental Protocol / "What Is Computed, and What Is Not" section) before trusting existing prose as a baseline to build on -- a section that already exists and reads fluently is not evidence it was ever true for this paper.
- **[R94]**: The automated gate is necessary but not sufficient: R-general (already stated in ERR-089/090/092 but worth restating precisely here) is that a clean run_submission_gate.py result does not mean a manuscript contains no fabricated claims, only that it contains no claim the current regex patterns can see. A human or agent expanding or auditing a section should read for empirical-sounding assertions -- 'our fit gives', 'achieves X performance', specific comparative claims -- and verify each against the paper's own stated methodology, independent of whether the gate flagged it. Separately: consider extending claim_provenance.py's _PATTERNS to catch bare LaTeX-variable-assigned numbers ($\text{var} = N$) as a follow-up, since this is the second time in this paper alone that phrasing has hidden an unsupported claim from the gate -- not done here, out of scope for the citation/expansion work this session is doing.
- **[R95]**: Given three confirmed instances across two of the nine manuscripts, a worthwhile next step (not done in this session, flagged for follow-up) would be to grep all nine drafts for 'H100', 'A100', 'GPU cluster', 'MMLU', 'GSM8K', 'HumanEval', and 'backbone' to check whether the same fabricated-hardware-benchmark boilerplate exists in the remaining papers this session did not touch (review_enterprise_genai_roi, review_spatio_temporal_grounding_in_video_question_answering) or in ones this session already passed over without a full line-by-line read (review_symbol_graph_rag_vs_qlora, review_enterprise_adoption, review_trustworthy_multi_agent_systems, autonomous_code_synthesis, review_continual_safety_alignment) -- this session's own edits to those five did not include a systematic grep for this specific pattern.
- **[R96]**: Extends R-general from ERR-093/094/095: do not rely on a keyword grep to find this defect class exhaustively -- a grep only proves the terms searched for are absent, not that the defect is. The reliable check is reading every Limitations/Threats/External Validity section against that same paper's own explicit statement of what it did and did not run (usually stated plainly in its Abstract, its 'What Is Measured' or equivalent protocol section, and its Conclusion) -- this session did that for all 9 manuscripts' Limitations sections specifically (not merely grepped), and found exactly these 5 instances across 3 of the 9 papers (review_architectural_dynamics_long_12_page x2, review_composable_ai_systems_for_trustworthy_agentic_pipelines x1, review_spatio_temporal_grounding_in_video_question_answering x1, review_symbol_graph_rag_vs_qlora_swe_bench_lite x1); no further instances found across the remaining Limitations sections read this session.
- **[R97]**: Before reporting 'the dev server works' or 'I tested the UI', confirm the backend actually reaches its own startup print statement and listens on its port -- a running process (ps shows it alive) is not evidence it is working, especially on this filesystem. If a Python process shows near-zero CPU growth and produces no log output for more than ~30s, use `lsof -p <pid>` to see the last file it opened before assuming either 'it's fine, just slow' or 'it's hung' -- the actual blocking file is directly diagnostic (as it was here) and costs one command, not an indefinite wait.
- **[R98]**: On this venv, until it is recreated: always invoke tools as `backend/.venv/bin/python -m <module>` or `backend/.venv/bin/python <script.py>`, never via their installed console-script wrapper (`backend/.venv/bin/<toolname>` directly) -- the wrapper's shebang is stale and will fail with a confusing 'No such file or directory' naming a path that looks like it should exist but doesn't. `python -m pip` already carries this exact workaround per the existing ERR-063 note; the same workaround now applies to every console script in this venv, not just pip.
- **[R99]**: When adding prose that discusses a result already shown in a table/figure, refer to it qualitatively (by name, by relative comparison, by pointing at the table) rather than re-typing its specific numeric value — even a correct, already-grounded value can fail the extractor's independent resolution. Only introduce a number via resync_manuscripts.py --apply from measurements.jsonl, never by hand, including when the number already appears elsewhere in the same manuscript.
- **[R100]**: A fabricated-content sweep scoped to "Limitations/Threats" sections is not sufficient — Contributions/Abstract lists are just as capable of asserting an experiment that was never run and must be checked against the same paper's own Methods/Ablation section, not assumed clean because the Limitations section already was.
- **[R101]**: Same as ERR-100/R100: check Contributions and Abstract sections for unrun-experiment claims with the same scrutiny applied to Limitations sections, not just the latter.
- **[R102]**: Before running anything that calls GEMINI_API_KEY (graphify extraction, CouncilOrchestrator research/debate regeneration), check whether another Gemini-dependent task already ran today, since the free tier's daily request cap (20/day for gemini-3-flash, confirmed via a live 429 RESOURCE_EXHAUSTED response) is shared across every feature using that key, not per-feature. Do not run graphify extraction and debate/research regeneration in the same session without spacing them across quota-reset windows, and never scope a graphify run at a folder that also contains generated build artifacts (exports/, preview_tiles/) -- point it at the specific manuscript files instead.
- **[R103]**: When 'API failure or quota reached' placeholder text appears anywhere in the vault, do not assume it is only a Gemini quota issue (ERR-102) -- the GROQ and NIM fallbacks behind it are independently broken as of this recording. Before trusting the fallback chain in council.py/meta_review_council.py/venue_advisor.py again, verify llama-3.1-8b-instant still exists on GROQ and pick a current, non-EOL NIM model to replace meta/llama-3.1-8b-instruct, updating all call sites (ideally centralizing the model ID instead of repeating it four times).
...
### ❌ [ERR-089] p1 scores BM25 and Symbol+PPR on the same queries (a paired design) but computed significance with welch_t(), an independent two-sample test (df = n_a + n_b - 2 = 328), while the manuscript Methods section describes the comparison as a paired difference. The recorded degrees of freedom never matched a paired test on this data (df should be n - 1 = 171).
- **Timestamp:** `2026-09-03 13:54:51`
- **Component:** `scripts/experiments/harness.py welch_t() + p1_symbol_graph_retrieval.py` (statistical_significance_test)
- **Error Type:** `Wrong Test For Design -- Independent Test Applied To Paired Data`
- **Root Cause:** The shared harness helper welch_t() only implements scipy.stats.ttest_ind (independent samples). The one caller whose data is actually paired (p1, same queries under two systems) used it anyway; the only other caller (p5, independent Monte Carlo samples across different topologies) is a correct use of the same function.
- **Resolution:** Added ErrorLedgerService-adjacent paired_t() to harness.py (scipy.stats.ttest_rel, df = n - 1, Cohen's dz on the per-query differences) as a new method -- welch_t() is untouched so p5's correct usage is unaffected. p1_symbol_graph_retrieval.py now records both (welch_t kept for transparency, paired_t as the headline statistic) and the manuscript's Methods, Results, and Conclusion sections were corrected to report the paired statistic and describe it accurately.
- **Prevention Rule:** `R89: Before recording a significance test, check whether the two samples are the same units measured twice (paired -- use ttest_rel / a paired effect size) or genuinely independent observations (use ttest_ind). A manuscript sentence describing the comparison as "paired" is a claim about method (HANDOFF.md: "a claim about method is still a claim") -- verify the code actually implements what the prose says, not just that a number came from somewhere.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-090] 7 measurements backing the manuscript's "Where Does Structural Retrieval Change the Ranking?" section (queries_unchanged_by_diffusion, queries_improved_by_diffusion, queries_degraded_by_diffusion, diffusion_inert_rate, bm25_top1_queries, bm25_top1_damaged_by_diffusion, bm25_missed_recovered_by_diffusion) existed only in the previously-recorded measurements.jsonl. Neither p1_symbol_graph_retrieval.py nor p1b_swebench_retrieval.py (the two scripts sharing this run_id) contains any code that computes them. Re-running p1 (to add an unrelated paired-test fix) triggered ExperimentRecorder's own-namespace cleanup, which correctly detected these 7 rows were not re-produced by the run and dropped them -- surfacing that the section's evidentiary basis had already been silently unreproducible before this session touched anything.
- **Timestamp:** `2026-09-03 13:54:51`
- **Component:** `scripts/experiments/p1_symbol_graph_retrieval.py / ExperimentRecorder own-prefix cleanup` (experiment_measurement_recording)
- **Error Type:** `Silent Data Loss -- Analysis Section Metrics No Longer Computed By Any Script`
- **Root Cause:** These 7 numbers were, at some point, computed once (by hand or by code since removed) and written into measurements.jsonl and the manuscript, without the computing logic ever being committed as part of either script. Nothing would have caught this until the next time the experiment was re-run, which had not happened since 2026-08-27.
- **Resolution:** Added real code to p1_symbol_graph_retrieval.py that derives all 7 metrics from the per-query MRR values already saved in retrieval_results.json (MRR = 1/rank for a single query, so exact rank -- and hence rank-change classification -- is recoverable without re-scoring). Re-ran p1: 30/30 measurements now recorded, 0 dropped. Manuscript resynced.
- **Prevention Rule:** `R90: A measurement with no code in the current tree that produces it is not evidence, it is a liability -- ExperimentRecorder's drop-and-warn behavior on re-run is the correct design (HANDOFF.md: "if a claim cannot be measured on this hardware, delete it. Do not estimate"). Re-running every experiment at least once per session before trusting its manuscript section is the only way this class of defect surfaces before submission.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ⚠️ [ERR-091] test_resync_manuscripts.py inserts scripts/experiments at index 0 of sys.path (to reach its own harness.py, the ExperimentRecorder module) and never removes it. If pytest collects that file before a test needing conftest.py's _isolate_harness_memory fixture (which does "from harness import continual_memory", expecting the unrelated backend harness/ package) in the same process, the fixture imports scripts/experiments/harness.py instead and fails with ImportError: cannot import name 'continual_memory'. Reproduces reliably when the two files are explicitly selected together; does not reproduce when run individually or via the full backend/tests/ -q collection (whose natural order avoids it).
- **Timestamp:** `2026-09-03 13:54:51`
- **Component:** `backend/tests/test_resync_manuscripts.py + backend/tests/conftest.py` (pytest_collection)
- **Error Type:** `Test Isolation -- Shared Module Name Collision Via Mutated sys.path`
- **Root Cause:** Two unrelated modules are both importable under the bare name "harness" (scripts/experiments/harness.py and a backend/harness package), and test_resync_manuscripts.py mutates global sys.path state without a fixture-scoped teardown to restore it, so the collision is order-dependent rather than deterministic.
- **Resolution:** Not fixed -- out of scope for the citation-backlog/main-body-expansion/SKILL.md work this session was doing. Confirmed pre-existing (unrelated to any content edit this session made) and confirmed the standard invocation, pytest backend/tests/ -q (what CI runs and what this session used throughout), is not observed to hit it.
- **Prevention Rule:** `R91: Any test that inserts into sys.path must remove it via a fixture with teardown (monkeypatch.syspath_prepend, not a bare sys.path.insert), or must import under a qualified/aliased name instead of a bare module name that collides with another package in this repo. Until fixed, never hand-select an arbitrary subset of test files by path -- always run backend/tests/ -q as a whole, or single files in isolation.`
- **Status:** ⚠️ `OPEN`

### ❌ [ERR-092] Same pattern as ERR-090, second paper: 4 pairs of measurements backing the "Containment Is Bimodal, Not Graded" section (trials_fully_contained_{mesh,contract_net,blackboard,hierarchical}, trials_catastrophic_{mesh,contract_net,blackboard,hierarchical}) existed only in the previously-recorded measurements.jsonl with no code in the current script computing them. Surfaced while adding an unrelated closed-form sanity-check metric.
- **Timestamp:** `2026-09-03 14:34:04`
- **Component:** `scripts/experiments/p5_coordination_topologies.py` (experiment_measurement_recording)
- **Error Type:** `Silent Data Loss -- Analysis Section Metrics No Longer Computed (second instance of ERR-090)`
- **Root Cause:** Same as ERR-090 -- computed once, never committed as part of the script.
- **Resolution:** Added code computing all 8 metrics directly from the per-trial cascade_samples arrays already held in memory during the Monte Carlo run (np.mean(samples < 0.05), np.mean(samples >= 0.5)). Re-ran p5: values matched the existing manuscript prose exactly (27.94, 63.47, 96.18, 0.92 confirmed), so this also serves as an independent check that the original numbers were correct, not just recoverable. Also caught and fixed a real hand-arithmetic error in a newly-added sentence in the same edit: 1-(1-0.02)^64 written as 72.43% by hand, actual computed value 72.55%.
- **Prevention Rule:** `R92: Given this is now 2 for 2 papers checked, treat every p1-p9 script as likely to have orphaned analysis metrics until each has been re-run and diffed against its manuscript at least once this cycle -- do not assume a script with a passing gate has no silently-unreproducible content. This also reconfirms R-general from ERR-089/ERR-090: any number typed directly into prose during editing, even simple closed-form arithmetic, must be verified by an actual computation before being trusted, not by hand.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-093] The Limitations section described accuracy on MMLU/GSM8K/HumanEval, scaling laws fitted with R^2>0.988 to empirical data, and an NVIDIA H100/A100 GPU cluster target. The rest of this same paper explicitly states no hardware measurement was made and no benchmark was run (Experimental Protocol section: this is closed-form arithmetic plus simulation on synthetic matrices, CPU-only). Found while writing the missing Experiments section this paper needed.
- **Timestamp:** `2026-09-03 21:00:17`
- **Component:** `vault/04_Drafts/review_architectural_dynamics_long_12_page.md, Limitations section` (manuscript_content_review)
- **Error Type:** `Fabricated/Stale Claims -- Described Benchmarks And Hardware Never Used`
- **Root Cause:** Almost certainly boilerplate carried over from an earlier draft or a different paper template that assumed real benchmark evaluation, never reconciled with this paper after its methodology was narrowed to exact arithmetic and simulation only.
- **Resolution:** Rewrote the Limitations section: added an explicit correction note naming what the prior text wrongly described, then replaced each of the three bullets with a limitation actually applicable to a closed-form-arithmetic-and-simulation study (hardware independence of exact arithmetic vs. construction-dependence of simulated results; inherited vs. re-derived task-distribution dependence of the cited Chinchilla relationship; assumption risk vs. regression-extrapolation risk for frontier-scale behaviour).
- **Prevention Rule:** `R93: When expanding or auditing any section of a manuscript, cross-check its claims against the paper's own stated methodology (its Experimental Protocol / "What Is Computed, and What Is Not" section) before trusting existing prose as a baseline to build on -- a section that already exists and reads fluently is not evidence it was ever true for this paper.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-094] The compound-architecture extension asserted 'our empirical fit gives lambda = 8.3' and 'Symbol-RAG with a 14B parameter model achieves MMLU performance exceeding a 70B dense model.' This study runs no benchmark and fits no scaling law to empirical data (stated in its own Experimental Protocol section); no MMLU comparison of any kind was run. This is the same class of defect as the Limitations section fix (ERR-093) in the same paper, but more serious: a specific false empirical result rather than stale boilerplate, and it was NOT flagged by run_submission_gate.py -- checked directly against submission_gate.json, no claim containing '8.3' or '46.2' was extracted at all.
- **Timestamp:** `2026-09-03 21:01:39`
- **Component:** `vault/04_Drafts/review_architectural_dynamics_long_12_page.md, FLOPs Scaling Law: Compound Architecture Extension` (manuscript_content_review)
- **Error Type:** `Fabricated Empirical Result -- Undetected By The Automated Claim Gate`
- **Root Cause:** Two compounding causes. (1) Same as ERR-093: the manuscript prose asserted more than this CPU-only study's methodology supports, likely leftover from an earlier, differently scoped draft. (2) A real gap in ClaimProvenanceService's regex-based claim extractor: '$\lambda = 8.3$' (a bare number after a LaTeX Greek-letter variable, no recognized unit) and '$\sim 46.2\times$' (a factor pattern requiring the literal spelling '$\times$' that this occurrence's exact LaTeX spacing did not match) both fell outside every extraction pattern in claim_provenance.py's _PATTERNS tuple, so the gate had zero chance of catching this regardless of grounding -- it never became a claim to check.
- **Resolution:** Rewrote the paragraph: kept the symbolic loss-decomposition extension (a legitimate theoretical construction), removed the fabricated lambda value and the fabricated model comparison, and added an explicit correction note. Manually scanned the rest of the manuscript for the same pattern (grep for 'our empirical', 'our benchmark', 'MMLU', 'achieves...performance', etc.) -- no further instances found in this paper.
- **Prevention Rule:** `R94: The automated gate is necessary but not sufficient: R-general (already stated in ERR-089/090/092 but worth restating precisely here) is that a clean run_submission_gate.py result does not mean a manuscript contains no fabricated claims, only that it contains no claim the current regex patterns can see. A human or agent expanding or auditing a section should read for empirical-sounding assertions -- 'our fit gives', 'achieves X performance', specific comparative claims -- and verify each against the paper's own stated methodology, independent of whether the gate flagged it. Separately: consider extending claim_provenance.py's _PATTERNS to catch bare LaTeX-variable-assigned numbers ($\text{var} = N$) as a follow-up, since this is the second time in this paper alone that phrasing has hidden an unsupported claim from the gate -- not done here, out of scope for the citation/expansion work this session is doing.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-095] External Validity claimed 'lower-capacity backbones (<=3B parameters) exhibit higher initial schema violation rates' and 'Benchmarks were conducted on NVIDIA H100 and A100 GPU clusters.' This paper's own Empirical Evaluation Protocol states 'No language model is invoked at any point; stages are contracts, not agents,' and the Conclusion repeats it. No backbone was tested at any size, and no GPU cluster was used. This is the third manuscript in this session found with this exact defect shape (ERR-093, ERR-094 were the first two, both in review_architectural_dynamics_long_12_page.md).
- **Timestamp:** `2026-09-03 21:03:56`
- **Component:** `vault/04_Drafts/review_composable_ai_systems_for_trustworthy_agentic_pipelines.md, Threats to Validity & Limitations` (manuscript_content_review)
- **Error Type:** `Fabricated/Stale Claims -- Third Instance Of The ERR-093/094 Pattern`
- **Root Cause:** Same as ERR-093/094: stale or template-inherited prose asserting hardware/benchmark activity the study's own stated methodology disclaims. Occurring identically in a third, otherwise unrelated manuscript raises the likelihood this is a systemic artifact of however these Limitations/Threats sections were originally drafted or generated across the whole 9-paper set, not an isolated one-off in a single paper.
- **Resolution:** Rewrote the Internal/External Validity bullets: added an explicit correction note, removed the fabricated backbone-dependence and GPU-cluster claims, replaced with limitations actually applicable to a contract-algebra study that executed no agent (whether real agents honour their declared contracts; whether six typed properties capture a real system's full pre/post-condition surface).
- **Prevention Rule:** `R95: Given three confirmed instances across two of the nine manuscripts, a worthwhile next step (not done in this session, flagged for follow-up) would be to grep all nine drafts for 'H100', 'A100', 'GPU cluster', 'MMLU', 'GSM8K', 'HumanEval', and 'backbone' to check whether the same fabricated-hardware-benchmark boilerplate exists in the remaining papers this session did not touch (review_enterprise_genai_roi, review_spatio_temporal_grounding_in_video_question_answering) or in ones this session already passed over without a full line-by-line read (review_symbol_graph_rag_vs_qlora, review_enterprise_adoption, review_trustworthy_multi_agent_systems, autonomous_code_synthesis, review_continual_safety_alignment) -- this session's own edits to those five did not include a systematic grep for this specific pattern.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-096] Two more instances of the same defect family (ERR-093/094/095), found by a broader grep for 'Our benchmark'/'Our evaluation' rather than hardware/dataset names, which is why they were missed by the first sweep in ERR-095: (1) review_spatio_temporal_grounding's Limitations claimed 'Our benchmarks evaluate video clips up to 5 minutes' and 'Our evaluation focuses exclusively on visual and textual modalities' -- this paper explicitly and repeatedly states no benchmark or evaluation was run at all (Section 5 title is literally 'Proposed Evaluation, and Why It Is Not Reported'). (2) review_symbol_graph_rag_vs_qlora's Limitations claimed 'Our evaluation spans repositories up to 85,000 lines of code' -- no lines-of-code metric exists anywhere in this paper's 37 recorded measurements; the only related recorded fact is a median of 198 Python files per repository. Both again evaded run_submission_gate.py because the claim extractor has no unit pattern for 'lines of code' and no pattern that would flag '5 minutes' or 'T <= 64 sampled frames' as an unsupported duration claim.
- **Timestamp:** `2026-09-03 21:06:15`
- **Component:** `vault/04_Drafts/review_spatio_temporal_grounding_in_video_question_answering.md (External Validity) and vault/04_Drafts/review_symbol_graph_rag_vs_qlora_swe_bench_lite.md (Limitations, repository scale)` (manuscript_content_review)
- **Error Type:** `Fabricated/Stale Claims -- 4th and 5th Instances Of ERR-093 Pattern, Different Signature`
- **Root Cause:** Same as ERR-093/094/095. The first sweep for this family (ERR-095) searched for 'H100|A100|GPU cluster|MMLU|GSM8K|HumanEval|backbone' and missed these because neither contains those terms -- the actual, more general signature is any 'Our benchmark(s)' or 'Our evaluation' phrasing in a paper whose own stated methodology ran no benchmark or evaluation. A term-list grep is inherently incomplete for this defect class; only reading each Limitations/Threats section against its own paper's stated scope reliably catches it.
- **Resolution:** Fixed both: spatio-temporal's two bullets replaced with limitations about what the theory and architecture specification can and cannot establish (Theorem 1 gives an asymptotic bound, not a finite duration threshold; the architecture has no audio pathway defined at all, so audio fusion needs a model change before an experiment). symbol_graph_rag_vs_qlora's bullet corrected to cite the actual recorded figure (median 198 files/repo) instead of the fabricated LOC count.
- **Prevention Rule:** `R96: Extends R-general from ERR-093/094/095: do not rely on a keyword grep to find this defect class exhaustively -- a grep only proves the terms searched for are absent, not that the defect is. The reliable check is reading every Limitations/Threats/External Validity section against that same paper's own explicit statement of what it did and did not run (usually stated plainly in its Abstract, its 'What Is Measured' or equivalent protocol section, and its Conclusion) -- this session did that for all 9 manuscripts' Limitations sections specifically (not merely grepped), and found exactly these 5 instances across 3 of the 9 papers (review_architectural_dynamics_long_12_page x2, review_composable_ai_systems_for_trustworthy_agentic_pipelines x1, review_spatio_temporal_grounding_in_video_question_answering x1, review_symbol_graph_rag_vs_qlora_swe_bench_lite x1); no further instances found across the remaining Limitations sections read this session.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-097] backend/main.py (run as `backend/.venv/bin/python backend/main.py`, the correct invocation -- see the separate uvicorn-shebang defect below) produces zero output, not even its own pre-uvicorn.run() print statement, for minutes. lsof on the running process showed it deep inside importing langsmith's auto-generated OpenAPI client (backend/.venv/lib/python3.13/site-packages/langsmith/_openapi_client/types/, hundreds of individual small .py/.pyc files, one per API schema type), advancing roughly one file per 10-20 seconds. At that rate a full import could take 30+ minutes to hours. langsmith is not imported by any of this project's own backend/*.py files (grepped directly, zero hits); it is pulled in transitively by langchain_core's tracer modules (langchain_core/tracers/context.py, langchain.py, evaluation.py, schemas.py, document_loaders/langsmith.py), which some backend dependency imports eagerly at module load time, well before the FastAPI app or uvicorn.run() is reached.
- **Timestamp:** `2026-09-03 22:08:04`
- **Component:** `backend/main.py startup / backend/.venv (langchain_core -> langsmith transitive import)` (dev_server_boot)
- **Error Type:** `Practically Unbootable Dev Server -- Import-Time I/O Cost, Not A Code Bug`
- **Root Cause:** Two compounding causes, same as every other I/O-latency finding this session: (1) this repo lives on iCloud Drive, and each of langsmith's hundreds of tiny generated-client files pays iCloud's per-file on-demand materialization cost on a cold read; (2) langchain_core eagerly imports its langsmith tracing integration as a side effect of being imported at all, rather than lazily on first actual use of tracing -- on ordinary local disk this is invisible (a few hundred ms), but on this filesystem each of those hundreds of files individually pays the iCloud materialization tax, compounding into a many-minutes-to-hours delay.
- **Resolution:** Fixed. Added backend/_langsmith_stub.py, which pre-registers minimal stub modules in sys.modules for langsmith, langsmith.run_helpers, langsmith.utils, langsmith.evaluation, and langsmith.evaluation.evaluator before anything else can import the real package. The three functions langchain_core actually calls at runtime (tracing_is_enabled, get_tracing_context, get_tracer_project) return correct tracing-is-off values; every other name langchain_core imports at module level (Client, RunTree, get_tracing_context, run_trees, EvaluationResult, EvaluationResults, and any future ones) resolves through a generic no-op stub via a module-level __getattr__ fallback, so the fix does not need to track langsmith's full API surface by hand. Wired in as the literal first import in backend/main.py. Verified: importing langchain_core.tracers.context (the module that previously hung for 30+ min importing the real langsmith._openapi_client tree) now completes in 0.56s, and a full python backend/main.py boot reaches application startup complete within seconds.
- **Prevention Rule:** `R97: Before reporting 'the dev server works' or 'I tested the UI', confirm the backend actually reaches its own startup print statement and listens on its port -- a running process (ps shows it alive) is not evidence it is working, especially on this filesystem. If a Python process shows near-zero CPU growth and produces no log output for more than ~30s, use `lsof -p <pid>` to see the last file it opened before assuming either 'it's fine, just slow' or 'it's hung' -- the actual blocking file is directly diagnostic (as it was here) and costs one command, not an indefinite wait.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-098] Running `backend/.venv/bin/uvicorn ...` directly fails immediately: '.../backend/.venv/bin/python3: No such file or directory'. The script's shebang line hardcodes the venv's original creation path, which was under a since-removed iCloud conflict-duplicate directory: '/Users/aryamandev/Library/Mobile Documents/com~apple~CloudDocs/Projects 2/ResearchingOS/backend/.venv/bin/python3'. This is the exact venv-shebang defect ERR-063 already named generically ('pip fails and python -m pip is used'), but confirmed here to also break `uvicorn` -- and by the same mechanism, every other console-script entry point in this venv (pytest, black, mypy, etc. if invoked by their installed script rather than via `python -m`).
- **Timestamp:** `2026-09-03 22:08:29`
- **Component:** `backend/.venv/bin/uvicorn` (dev_server_boot)
- **Error Type:** `Broken venv Console Script -- Stale Shebang Path (ERR-063 family, new concrete symptom)`
- **Root Cause:** pip/uv wrote an absolute shebang path at venv-creation time pointing into the then-current 'Projects 2' iCloud duplicate location. That duplicate no longer exists (confirmed removed earlier this session), so any console-script entry point in this venv is broken, while `backend/.venv/bin/python <script.py>` still works fine because invoking the interpreter binary directly bypasses the broken shebang entirely.
- **Resolution:** Fixed. Bulk-replaced the stale absolute shebang/exec path ('.../Projects 2/ResearchingOS/backend/.venv/bin/python3') with the correct current path ('.../Projects/ResearchingOS/backend/.venv/bin/python3') across all 33 affected console scripts in backend/.venv/bin/ via sed, plus activate/activate.fish/activate.csh. Also found and removed a genuine iCloud sync-duplicate file, backend/.venv/bin/websockets 2 (confirmed byte-identical to websockets via diff before deleting). Verified via grep returning no remaining matches for the stale path, and by directly running uvicorn --version, pip --version, pytest --version as their installed console scripts (not via python -m) -- all three now succeed. This directory is gitignored (venvs are never committed), so the fix is local/environmental only.
- **Prevention Rule:** `R98: On this venv, until it is recreated: always invoke tools as `backend/.venv/bin/python -m <module>` or `backend/.venv/bin/python <script.py>`, never via their installed console-script wrapper (`backend/.venv/bin/<toolname>` directly) -- the wrapper's shebang is stale and will fail with a confusing 'No such file or directory' naming a path that looks like it should exist but doesn't. `python -m pip` already carries this exact workaround per the existing ERR-063 note; the same workaround now applies to every console script in this venv, not just pip.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-099] New prose that re-typed an already-tabulated figure (Table 4, uncorrected-scheme fraction-of-maximum = 59.0%) as a fresh sentence caused the gate to report it UNGROUNDED, even though the identical value already appears in Table 4 and the underlying measurement is grounded there.
- **Timestamp:** `2026-09-04 16:49:07`
- **Component:** `claim_provenance.py (claim extractor)` (main-body expansion, architectural_dynamics)
- **Error Type:** `false-BLOCKED on a correct, already-grounded number`
- **Root Cause:** The claim extractor resolves each numeric mention independently against measurements.jsonl; a hand-typed restatement in new prose is not guaranteed to match the extractor's pattern/precision expectations even when the value is correct and already grounded elsewhere in the same document. This is the general case HANDOFF.md's ERR-052 already warns about (never hand-type a number) extended to restatements of numbers the document itself already contains, not just genuinely new ones.
- **Resolution:** Removed the specific restated figure from the new sentence and referred to the table row qualitatively instead ("Table 4's uncorrected-scheme row" rather than "...reaching only 59.0% of maximum entropy"). Gate re-run: 148/148 grounded, GATE: PASSED.
- **Prevention Rule:** `R99: When adding prose that discusses a result already shown in a table/figure, refer to it qualitatively (by name, by relative comparison, by pointing at the table) rather than re-typing its specific numeric value — even a correct, already-grounded value can fail the extractor's independent resolution. Only introduce a number via resync_manuscripts.py --apply from measurements.jsonl, never by hand, including when the number already appears elsewhere in the same manuscript.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-100] Contribution 5 claimed an ablation over "state immutability" and "consensus routing under induced network delays and adversarial hallucinations." The paper's own Ablation of the Checking Regime section and Empirical Evaluation Protocol state plainly that only pipeline depth and static-vs-runtime checking were varied, no agent was executed, and Tiers 1/2/4 (which include consensus routing) are unmeasured. Same defect family as the already-corrected Limitations sections (fabricated H100/A100 benchmarks, backbone comparisons) in this and sibling papers, but this instance was in the Contributions list, not Limitations, and was missed by the prior sweep because that sweep specifically targeted Limitations/Threats sections.
- **Timestamp:** `2026-09-04 16:51:38`
- **Component:** `vault/04_Drafts/review_composable_ai_systems_for_trustworthy_agentic_pipelines.md` (main-body expansion, composable_ai_systems)
- **Error Type:** `overclaim in Principal Contributions list, inconsistent with paper's own Ablation/Limitations sections`
- **Root Cause:** The prior citation/content-integrity pass (HANDOFF.md item 5) read every Limitations/Threats section against the paper's own stated scope but did not apply the same read to Introduction/Contributions sections, where an equally concrete but differently-located overclaim survived.
- **Resolution:** Rewrote Contribution 5 to describe exactly what Table 1/Table 2 and the Ablation of the Checking Regime section measure (depth x static/runtime checking), explicitly stating no ablation over backbones, task workloads, or Tiers 1/2/4 was performed, with the correction noted inline the same way the sibling Limitations corrections are.
- **Prevention Rule:** `R100: A fabricated-content sweep scoped to "Limitations/Threats" sections is not sufficient — Contributions/Abstract lists are just as capable of asserting an experiment that was never run and must be checked against the same paper's own Methods/Ablation section, not assumed clean because the Limitations section already was.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ❌ [ERR-101] Contribution 5 ("Comprehensive Ablation and Failure Mode Analysis") claimed to isolate the performance impact of temporal residual scaling, frame sampling density, and orthogonal manifold constraints under adversarial temporal shuffling. This paper has zero measurements of any kind (0 claims in the provenance gate) and Contribution 4, immediately above it in the same list, explicitly states no experimental results are reported. Found by spot-checking all 9 manuscripts' Contributions lists immediately after finding the same pattern in composable_ai_systems (ERR-100).
- **Timestamp:** `2026-09-04 16:52:31`
- **Component:** `vault/04_Drafts/review_spatio_temporal_grounding_in_video_question_answering.md` (spot-check of ERR-100 defect class across all 9 manuscripts)
- **Error Type:** `overclaim in Principal Contributions list, same family as ERR-100`
- **Root Cause:** Same as ERR-100: the prior fabricated-content sweep was scoped to Limitations/Threats sections and did not check Contributions/Abstract lists for the same class of unrun-experiment claims.
- **Resolution:** Rewrote Contribution 5 to state plainly that the listed ablations are specified (per Contribution 4's falsifiable protocol) but not run, matching the honest scope already stated in the Executive Abstract and Contribution 4.
- **Prevention Rule:** `R101: Same as ERR-100/R100: check Contributions and Abstract sections for unrun-experiment claims with the same scrutiny applied to Limitations sections, not just the latter.`
- **Status:** ✅ `VERIFIED_RESOLVED`

### ⚠️ [ERR-102] graphify extract on vault/04_Drafts (43 semantic chunks) hit Gemini free-tier RESOURCE_EXHAUSTED errors starting at chunk 9, including an explicit GenerateRequestsPerDayPerProjectPerModel-FreeTier quota of 20 requests/day for gemini-3-flash -- a hard daily cap, not a per-minute one a short retry clears. This is almost certainly also why the original 9 vault/03_Debates placeholders all failed with 'API failure or quota reached' in an earlier session: the same GEMINI_API_KEY, the same 20-req/day ceiling. A same-day CouncilOrchestrator.run_debate_only() test (scripts/regenerate_debates.py) run immediately after was still stalled mid-Critique-stage with no further progress at time of recording, consistent with the same exhausted daily budget.
- **Timestamp:** `2026-09-04 17:20:15`
- **Component:** `Gemini API (GEMINI_API_KEY, free tier) -- shared by graphify semantic extraction and CouncilOrchestrator debate/research generation` (Thread 2: scoped debate regeneration + graphify extraction on vault/04_Drafts)
- **Error Type:** `shared daily API quota exhausted mid-task, blocking both graphify and debate regeneration`
- **Root Cause:** graphify (semantic extraction) and CouncilOrchestrator (research/debate/draft generation) both read GEMINI_API_KEY from the same .env and call the same gemini-3-flash free-tier endpoint, which caps at 20 requests/day project-wide regardless of which tool or feature is calling it. Running graphify's deep-mode extraction (43 chunks, one request each) first exhausted essentially the whole daily budget before the debate-regeneration test got to run its own handful of critique/debate/synthesis calls.
- **Resolution:** No code fix applies here -- this is an external quota limit, not a bug. Recorded so a future session does not re-diagnose 'API failure or quota reached' as a code defect (per HANDOFF.md's ERR-088 precedent for a different cause of a similar symptom) and instead checks quota/billing first. Also fixed an adjacent scoping bug found in this same run: pointing graphify at vault/04_Drafts swept up vault/04_Drafts/exports/ (108+ generated PDF/tex/bib files across 12 venues) and preview_tiles/ (hundreds of PNGs) alongside the 9 real manuscripts, burning quota on build artifacts instead of authored content -- graphify should be pointed at a path containing only the manuscript .md files, or exports/preview_tiles/backtest_runs should be excluded, when this is retried.
- **Prevention Rule:** `R102: Before running anything that calls GEMINI_API_KEY (graphify extraction, CouncilOrchestrator research/debate regeneration), check whether another Gemini-dependent task already ran today, since the free tier's daily request cap (20/day for gemini-3-flash, confirmed via a live 429 RESOURCE_EXHAUSTED response) is shared across every feature using that key, not per-feature. Do not run graphify extraction and debate/research regeneration in the same session without spacing them across quota-reset windows, and never scope a graphify run at a folder that also contains generated build artifacts (exports/, preview_tiles/) -- point it at the specific manuscript files instead.`
- **Status:** ⚠️ `OPEN`

### ⚠️ [ERR-103] When the primary Gemini call failed (daily free-tier quota exhausted, see ERR-102), the LLM router's fallback chain tried GROQ then NVIDIA NIM, and both failed for reasons unrelated to quota: GROQ returned 404 for model 'llama-3.1-8b-instant' ('does not exist or you do not have access to it'), and NIM returned 410 Gone for 'meta/llama-3.1-8b-instruct' ('reached its end of life on 2026-08-26T09:00:00Z and is no longer available'). Both model IDs are hardcoded in multiple places: llm_router.py lines 36/44/48/73/79/85, council.py lines 26/50/61/72, meta_review_council.py lines 47/59, and venue_advisor.py line 394. The practical effect observed live: when Gemini's daily quota is exhausted, every one of these call sites falls straight through to a 'structured placeholder inserted' response instead of a working fallback provider -- the fallback chain that HANDOFF.md's placeholder text points users at ('Re-run or switch providers via .env') does not currently work.
- **Timestamp:** `2026-09-04 17:21:28`
- **Component:** `backend/services/llm_router.py, backend/agents/council.py, backend/agents/meta_review_council.py, backend/agents/venue_advisor.py -- GROQ and NVIDIA NIM fallback provider model names` (Thread 2: debate-only regeneration test (scripts/regenerate_debates.py))
- **Error Type:** `hardcoded fallback-provider model IDs are stale/retired, both fallbacks fail before reaching Gemini`
- **Root Cause:** GROQ and NVIDIA NIM both retire/rename hosted model IDs over time; these two specific IDs were hardcoded across four files rather than centralized or configured, so both went stale together and there is no single place to update them. NIM's error message dates the retirement precisely (2026-08-26), meaning this fallback has been broken for at least several days as of this recording.
- **Resolution:** Not fixed -- deliberately left open rather than guessed. Picking a replacement model ID without checking GROQ's and NVIDIA NIM's current model catalogs risks hardcoding another name that goes stale the same way; that check needs a human or a session with time to verify current model availability against each provider's docs/console, not a guess made while unblocking an unrelated task.
- **Prevention Rule:** `R103: When 'API failure or quota reached' placeholder text appears anywhere in the vault, do not assume it is only a Gemini quota issue (ERR-102) -- the GROQ and NIM fallbacks behind it are independently broken as of this recording. Before trusting the fallback chain in council.py/meta_review_council.py/venue_advisor.py again, verify llama-3.1-8b-instant still exists on GROQ and pick a current, non-EOL NIM model to replace meta/llama-3.1-8b-instruct, updating all call sites (ideally centralizing the model ID instead of repeating it four times).`
- **Status:** ⚠️ `OPEN`
